...
<!-- gen:methods-version -->**Version 0.5.1** · reader edition · applies to the release built from the 2026-09-26 snapshot<!-- /gen:methods-version -->
...
This document defines every published number. The pipeline that builds the release is kept privately; where it and this document disagree, the pipeline is what gets corrected. Changing a definition, filter, threshold, or class break — validation thresholds included — requires a version bump and a changelog entry (§13). Sources cited are listed in §14.

**Reading it.** §0–§5 say what is counted and where; §6 defines each metric in `potholes_long.csv`; §7 says when a figure is withheld and how the map is colored; §8 is the file layout and the caveat text that travels with each flag; §10 is every validation check, whose results are in `qa_report.json`; §12 is what the release does not claim. The raw snapshots are not published; they are about 1 GB of City and Census records per pull. Every published figure is recomputable from the dated source snapshot (held privately), with the dataset IDs and parameters in §1.
...
**Snapshot rules**
- Every pull is saved as one dated snapshot, `snapshot_id` = UTC ISO timestamp of the pull (given in `qa_report.json`). The build reads only that snapshot and makes no network calls apart from the V1e warning's live query (§10).
- At pull time, the reconciliation aggregate queries in V1 are run against the portal and their results saved with the snapshot, V1c's two citywide totals with the UTC time they were taken. Each is taken once; resuming a pull never re-queries it.
...
### 4.4 Road class
Snap each point to the nearest `pr57-gg9e` segment within 100 ft, considering all classes except `RIV`. If two or more segments are exactly the same distance away, the one with the lowest segment id wins, as §4.2 and §4.5 already resolve ties for polygons. At the 2026-09-21 snapshot, 65 points in the 311 data sit at an exact tie and 15 of those are tied across two road classes, so without the rule a rebuild could move them between `local` and `other_unsnapped` and V11a would fail.
...
**Snap coverage (D6).** METHODS sets a target of ≥ 97% of points snapped within 100 ft. At the 2026-09-21 snapshot 93.69% of 311 points and 93.7% of Patched points snap, so D6 is **not met** and is published as such. The 100 ft radius is kept. The 6.31% unsnapped are `other_unsnapped`: they stay in every per-mile numerator (they are not known to be limited-access) and drop out of the `__arterial` / `__local` variants, which therefore cover about 94% of reports.
...
- A clipped piece that lies on a shared boundary (returned for more than one polygon of the same `geo_type`) counts only for the lowest numeric `geo_id`. Pieces inside no polygon count toward `UNASSIGNED`. Without this rule, the 2026-09-16 snapshot double-counted about 92 mi on community-area boundaries (Σ CA about 4,010 mi vs. about 3,918 mi for their union) and failed V9.
- The segments' own `length` field sums to 3,945.2 mi at the 2026-09-16 snapshot: 422.7 mi (class 2), 473.7 mi (class 3), and 3,048.8 mi (class 4). That is the network before Lake Shore Drive is added (next rule) and before clipping. The denominator is the clipped geometric length: Lake Shore Drive's segments bring the unclipped total to 3,977.2 mi, and clipping to the city leaves 3,950.4 mi.
...
| `YTD{YYYY}-{MM}` | `YYYY-01-01T00:00:00 ≤ t < first day of month MM+1 in YYYY` | `MM` is the last full calendar month that ended ≥ 7 days before `data_currency`; the same `MM` is used for every year 2019–current. At the 2026-09-26 snapshot `MM = 08`, and `YTD{YYYY}-08` holds 82.9% of 2019–2025 reports (167,565 of 202,011 `rpt_n`, ward-office reports included). |
...
- `geo_denominators.csv`: `geo_type, geo_id, street_mi, pop2020, poverty_rate, lep_hh_share, broadband_hh_share, pct_hispanic, pct_nh_black, pct_nh_white, pct_nh_asian, acs_vintage, centerline_rows_updated`. Rows for the city, community areas, wards and, from 0.5, each poverty quintile and race group (§4.7 step 8), each non-city type with `UNASSIGNED` and strata with `EXCLUDED`. Units, since §8.1's precision rule covers `potholes_long.csv` alone: `street_mi` is miles to 4 decimals (§4.5); `pop2020` is persons (§4.6); `poverty_rate`, `lep_hh_share`, `broadband_hh_share` and every `pct_*` are **0–1 shares** to 6 decimals, not percentages, and are the §4.7 step 6 covariates — empty on `UNASSIGNED` and `EXCLUDED`; `acs_vintage` is the ACS 5-year vintage as a year; `centerline_rows_updated` is the centerline dataset's Socrata `rowsUpdatedAt`, written as an ISO-8601 UTC timestamp. **The `UNASSIGNED` row is not a place:** its `street_mi` is city mileage lying inside no polygon of that `geo_type` and its `pop2020` is the 2020 population of Cook County outside the city (§4.7 step 8), so an area total is the sum of the real areas — `EXCLUDED` included for strata — with `UNASSIGNED` dropped. V9 allows those sums 0.5% against the city row; at 0.5 community areas and strata land within 0.01% and wards 0.29% low, the ward boundaries not tiling the community-area union exactly.
...
- `facts.json`: every figure the project page states, one entry per figure with `value` (the exact figure), `display` (the string the page prints, thousands separators and unit included), `label`, a one-sentence `definition`, `source_file`, `sources`, `rounding` and `unit`; written at export from the tables above and the deterministic parts of `qa_report.json` (join loss, the Gate 0 diagnostics), never from the database. The top level carries the release date, `snapshot_id`, `data_currency`, `methods_version` and the coverage window. A percent is on a 0–100 scale; distances are in feet; no entry carries a metric unit.
- `community_area_summary.csv`, `ward_summary.csv`: one row per community area (77) and per ward (50), `UNASSIGNED` excluded, for the latest complete calendar year (`default_period`): `geo_id, geo_name, period`, then `rpt_n`, `rpt_per_mi`, `close_p50_d`, `rpt_patched_share` and `fill_per_mi`, each followed by its `n` and `flags` columns. The value text is exactly what `potholes_long.csv` prints for that row; a withheld cell stays empty with its `n` and flag intact. Reshaped from the long table; nothing is recomputed.

All companion tables are listed in `checksums.sha256` under the existing release contract. The map's files, and the two JavaScript libraries it loads, are listed in `site/checksums.sha256`. From 0.5.1 they include `site/data/city_series.json`: the `city` rows of `potholes_long.csv` that the winter chart draws (`rpt_n` and `rpt_yoy_pct` at single-year CY and YTD periods, `close_p50_d` at YTD periods, `patch_p50_d` at CY periods), each with its `n` and flags, so the chart need not load the whole long table. `data_currency` is written in one form everywhere, `YYYY-MM-DDTHH:MM:SS` (`qa_report.json` wrote a space before 0.5.1).
...
- **C-WARD-PRE2023** — Years before 2023-05-15 are shown on today's ward map for comparison. This area was then split among different wards and alderpersons. These figures say nothing about any current alderperson.
- **C-2023** — 2023 straddles the ward remap. Reports before 2023-05-15 were made under the old boundaries.
...
- **C-WARDPOP** — Ward populations in the 2020 Census are all within 5% of the ward average, so a ward's reports per 1,000 residents track its raw report count closely.

C-WARDPOP belongs to no flag. The map shows it on the ward view of `rpt_per_1k` for as long as D7 (§10) finds every ward within 5% of the average; the population is `P1_001N` of the 2020 Census P.L. 94-171 redistricting data (§1, §4.6). `{Mon DD}` resolves in the same form as the window it ends, "Aug 31" (through 0.5, "Aug. 31").
...
**Status words.** Each gate writes its own words to `qa_report.json`, and the release's summaries select on them, so they are kept as they are:
- Gate 0: `met` or `not met` for a diagnostic with a target (D1–D3, D6, D9); `tripped` or `not tripped` for one whose trigger changes a rule (D5, D7); `report only` for the rest (D4, D8, D10).
- Gate 1: `PASS`, `FAIL` (a `FAIL` with `accepted` is a published failure), or `SKIPPED` when a check cannot run (V11 before export, V1c on a snapshot without its pull-time record, V9b and V12 without Census data). A skipped Gate 1 check blocks publishing.
- Gate 2: `WARN` or `ok`, or `SKIPPED` when a warning cannot run (W9 before the weather stage, V1e when the live portal does not answer). A warning never blocks.
...
| D7 | 2020 population deviation across `ward2023` | If max \|dev\| ≤ 5%, caveat C-WARDPOP (§8.3) goes on the ward reports-per-1,000 view: per-capita tracks the raw count on wards. **Result:** 4.97% → tripped; C-WARDPOP added at 0.5.1 |
...
| ID | Check | Threshold |
|---|---|---|
| V1 | **Source reconciliation**, every part against portal aggregates recorded at pull time (§1), so a rebuild of a snapshot gets the same result on any later day. (a) Patched `count(*)` and `sum(number_of_potholes_filled_on_block)` by completion year vs. the raw rows. (b) 311 `count(*)` by year × `sr_short_code` × `duplicate` × `status` vs. the raw rows. (c) The two citywide totals a reader can query in one step each, Σ `number_of_potholes_filled_on_block` by completion year and PHF rows by created year, vs. the snapshot. | (a), (b) exact; (c) exact for closed years, the current partial year within 0.5% |
...
| V3 | **Geography loss.** `UNASSIGNED` share of each count metric, each CY | ≤ 1.5% (warn at 1.0%). At 0.5, 12 cells fail and are published (2019 `rpt_n`, `rpt_all_n`, `staff_n`; `crew_n` 2021, 2022, 2026; each on both geographies); at 0.5.1, from the 2026-09-26 snapshot, the same 12 cells fail and are published |
...
| V7 | **KM correctness.** (a) Fixture tests match independently computed / `lifelines` values (Kaplan and Meier 1958). (b) For cohorts with `close_open_share < 0.005`, KM p50 is within 0.5 day of the completed-only p50. (c) Exact: in those cohorts, when every open report is censored after the median, KM p50 equals the closed order statistic at rank ⌈n/2⌉ with open reports counted in n. | exact / 0.5 day / exact. At 0.5, (b) fails in 11 of 2,375 cohorts; at 0.5.1, from the 2026-09-26 snapshot, in 12 of 2,398 (the same 11 and one more, a ward street-closing cohort for CY2024). Both are published, and (c) passes in all of them — the completed-only median is one rank lower, and adjacent closing times there are more than 0.5 day apart |
...
| V14 | **Change statistics.** Fixture tests for the weighted log-ratio se, Greenwood variance, the winsorized dispersion factor, the two-sided p-value and the Benjamini–Hochberg step-up match independently computed values, and Greenwood matches `lifelines` | 1e-9 |
...
| ID | Check |
|---|---|
| V1d | V1c's current partial year differs from its pull-time aggregate, within V1c's 0.5% tolerance: rows created or revised while the pull ran. Read from the snapshot |
| V1e | **Drift since the pull.** At build time the two V1c queries run against the live portal and are compared with the pull-time record under V1c's rule; the warning trips where V1c would fail (any change in a closed year, or the partial year beyond 0.5%) and lists every count that moved. The one network call in a build; if the portal does not answer, V1e is `SKIPPED` |
| V3w | Geography loss (V3) above 1.0% on a count-year that is within the 1.5% cap |
| REF | The pull-time V1c totals differ by more than 0.5% from a reference pull of the portal taken 2026-09-16 (years 2019–2025), a check on retroactive revision upstream. Read from the snapshot |
...
**Window and labeling.** A winter `W{Y}` is Nov 1–Apr 30, beginning in year Y−1 and ending in year Y (local time). It pairs with `CY{Y}` and `YTD{Y}-{MM}`.
...
- Reference distribution: the 30 winters W1992–W2021 (1991-11 through 2021-04, matching NOAA's 1991–2020 climate normals period; Arguez et al. 2012 describe the normals convention). Standardizing against 30 winters rather than the 8 published ones is what keeps a single wild winter from redefining "typical."
...
| 0.5 | 2026-09-21 | **Pre-showcase audit.** *Thresholds restored.* V3 back to ≤ 1.5% and D6 back to ≥ 97%. Both had been changed in the build configuration before 0.3 was released (V3 to 2.25%, D6 to 90%; 0.3's `qa_report.json` already tested V3 at 2.25%) so that the build would pass, with no version bump or changelog entry, while this document still stated the old values. Their failures are now published: V3 fails on 12 cells — 6 count-years, each over the cap on both community areas and wards — and D6 is not met (93.69%). *Accepted failures:* a Gate 1 failure can be published, cell by cell, instead of blocking; new cells still block (§10). *Checks that never ran:* V7, V11 and V14 now run in every build (V11 after export, V7c added as an exact check; V7b fails in 11 cohorts and is published); V15 re-scoped from never-built sentence templates to the caveat registry (§8.4); W1, W2 and W4 implemented; W7 reported per layer; Gate 0 results published in `qa_report.json`. Summaries count the 15 defined checks, not sub-checks. *Suppression:* p90s gated on `n` ≥ 100, not population (189 rows had been published from as few as 15 observations); `fill_per_rpt` gated like a share; the stratum rule covers YTD; V8 re-derives every rule from `n`. *Change test:* per-layer dispersion factor and Benjamini–Hochberg FDR control replace the fixed 1.96-se test (clear areas fall from 22–76% to 0–8% of each layer); legend footnote and C-CHANGE reworded; `dispersion` added to `change_summary.csv`. *Flags:* a row carries the union of its windows' flags (450 ward change rows gain `PRE2023_WARD`; YTD comparisons gain `YTD`). *2019:* one policy (§5) — published, flagged, and in no pooled window, comparison, class break, equity pool or diagnostic headline; the equity pool becomes `CY2020-2025`; `rpt_yoy_pct` blank against 2019 and for the partial year. *Breaks:* recomputed on 2020 onward; all 12 YTD sets built from their own windows; the guard checks every set, not only that the current month's key exists. *Equity:* stratum street miles and population (§4.7 step 8), `crew_per_mi`, `race_majority` in `strata_tracts.csv`, the merged stratum named. *Precision:* one formatter tests `_pct` / `_pts` before `_per_` / `_share`. *Disclosure:* Lake Shore Drive's inclusion (§4.5); patch time is close time on the linked subset (§3); §11 reworded from causal to associational language with citations; D10 published with and without 2019 and exact p-values; references (§14). *Documents:* map default is Response time here and in the map specification; the public repository gets this reader edition. *Audit follow-up (2026-09-22), no definition changed:* the release states the snapshot date (`snapshot_id`) and the data currency separately, having published `data_currency` as both; `acs_vintage` is written as a year, not `2023.0`, and `centerline_rows_updated` as an ISO-8601 UTC timestamp, not epoch seconds; `qa_report.json` `summary.gate2.open_ids` names the warnings themselves (`V1d`, `V3w`), not the Gate 1 checks they roll up to; W5 reports the `snapshot_id` it tested; the site manifest lists only files that ship. *Reproducibility:* §4.4 gives the nearest-segment snap the lowest-id tie rule §4.2 and §4.5 already use. The snap returns every segment at the winning distance and the winner was left to a sort that is not stable, so the 15 points tied across two road classes could move between builds; V11a caught it as two builds of one snapshot differing in `potholes_long.csv` and `change_summary.csv`. *Website facts (2026-09-23), no definition changed:* §8.2 adds `facts.json` and the two summary tables, derived at export from the tables already published; the release date is a field of its own, distinct from the snapshot date, and `CITATION.cff` states it. *Release 2026-09-24, no definition changed:* `facts.json` states V7b's open-share threshold (`km_open_share_pct`); the map's files and the two libraries it loads are published under `site/` with their own `site/checksums.sha256`, and V11a compares them between builds. Built from the 2026-09-21 snapshot without rerunning validation: V1c compares against the live portal, which has moved on since the snapshot. |
| 0.5.1 | 2026-09-26 | **A version for the 2026-09-22 republish, and a rule change to V1c.** *The 0.5 republish.* 0.5 was first published 2026-09-21 and republished 2026-09-22 with §4.4's lowest-id tie rule for the road-class snap. That rule is a definition change, so it should have carried a new version; the 0.5 row above called the republish "no definition changed". It moved 81 rows of `potholes_long.csv` and 3 of `change_summary.csv`, and with the same republish `geo_denominators.csv` and `qa_report.json` changed. The four files of the 2026-09-21 release it superseded (SHA-256): `change_summary.csv` `7f416e7ea02bd16a769ee39d534fcce320348adae8ded317b0c0893a0c4dfc47`; `geo_denominators.csv` `3eda71d7e1fb44f9ced117020e16267646bb8178468f2dc82e404eede2425151`; `potholes_long.csv` `539e1d4eb565026ead12e7f16b8f7274252d97ec5c983021a3c2cdcbe9486f73`; `qa_report.json` `1f178b02dbc64d60c60df614df5e7d639c5cb4f26cfeec984915bac5d954d1dd`. The other six files of that release are byte-identical to 2026-09-22's. *Rule change, V1c.* V1c compares the snapshot with the portal aggregates recorded at pull time (§1), closed years exactly and the partial year within 0.5%; through 0.5 it compared with the live portal, so a rebuild more than a couple of days after its pull failed on the calendar, not on the data. The live comparison is the new warning V1e, which never blocks, and REF now reads the pull-time record. V1d, V1e, V3w and REF are defined in §10, with each gate's status words. *Added:* caveat C-WARDPOP for the ward per-1,000 view, from D7 (§8.3); `site/data/city_series.json` (§8.2); `facts.json` gains the year-to-date window, D7's trigger and result, and the V3w, V1c/REF and W9 lines. *Wording, no definition changed:* the winter and year-to-date windows are written "Nov 1–Apr 30" and "Jan 1–{Mon DD}" (caveat C-YTD resolved as "Aug 31", not "Aug. 31"); C-2023 dates the remap 2023-05-15; C-CREW, C-ALLEY, §9 and §12 write "the City" for the government; V7a and V14 compare with independently computed values; `qa_report.json` writes `data_currency` as `YYYY-MM-DDTHH:MM:SS`, as the manifests do; `site/data/manifest.json` drops the unused `asof_label`; `facts.json` labels a year-to-date period "2025-01 to 2025-08", not "Jan–Aug 2025"; V7b lists every cohort outside its tolerance, not the first four. *Corrected:* the 0.5 row said V3 and D6 were loosened between 0.3 and 0.4, and V3's published reason in `qa_report.json` called the change 0.4-era; they were loosened before 0.3 was released, and both now say so. *Results, from the 2026-09-26 snapshot:* V3 fails on the same 12 cells as at 0.5, and V7b in 12 of 2,398 cohorts: the 11 of 0.5 and one more, a ward street-closing cohort for CY2024 (KM median 32.0 days against 31.1 for closed reports alone, with one of its 393 reports open). An AI agent (Claude) working on the release checked that cohort against the snapshot, and the owner accepted it on 2026-09-25; V7c passes in every cohort. Both failures are published; no threshold moved. *Breaks:* frozen as at 0.5; only their version field changes. |
...
- Menne, M. J., I. Durre, B. Korzeniewski, S. McNeill, K. Thomas, X. Yin, S. Anthony, R. Ray, R. S. Vose, B. E. Gleason, and T. G. Houston (2012). Global Historical Climatology Network - Daily (GHCN-Daily), Version 3. NOAA National Climatic Data Center. doi:10.7289/V5D21VHZ
